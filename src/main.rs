...
fn main() {
    let n = 1000;
    let client_param = 2.0; // lambda param for the Poisson distribution
    let service_param = 2.0; // ??
    let warmup_param = 2.0; // shape parameter (scale parameter is fixed)

    queue(n, client_param, service_param, warmup_param).expect("ah");
}
...
fn queue(
    ...
    let warmup = Pareto::new(1.0, warmup_param)?.sample(&mut rand::thread_rng());
    ...
    for (&client_arrival, service_time) in incoming_clients.iter().zip(service_times) {
        ...
        if client_arrival > last_departure {
            delay = 0.0;
            warmup = Pareto::new(1.0, warmup_param)
                .unwrap()
